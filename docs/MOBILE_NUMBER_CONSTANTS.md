...
3. Update the value:
   ```typescript
   export const MOBILE_NUMBER = '9752731062'; // Change this number
   ```
...
- `MOBILE_NUMBER` - Raw 10-digit number (e.g., '9752731062')
- `DISPLAY_PHONE` - Formatted for display (e.g., '+91 9752731062')
- `DISPLAY_PHONE_WITH_DASH` - With dash separator (e.g., '+91-9752731062')
- `TEL_LINK` - For tel: links (e.g., 'tel:+919752731062')
- `WHATSAPP_LINK` - Base WhatsApp link (e.g., 'https://wa.me/919752731062')
- `WHATSAPP_BOOKING_LINK` - WhatsApp with English booking message
- `WHATSAPP_BOOKING_LINK_HI` - WhatsApp with Hindi booking message
- `SCHEMA_TELEPHONE` - For structured data (e.g., '+91-9752731062')
...
```markdown
Contact us at +91 9752731062 (update this when MOBILE_NUMBER changes)
```
...
1. **Blog Markdown Files** (`src/content/blog/*.md`)
   ...
   - Pattern: Search for `+91 9752731062` and replace
...
find src/content/blog -name "*.md" -exec sed -i 's/9752731062/NEW_NUMBER/g' {} \;
